...
public class TcpOverUdpClient {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Integer localTcpProxyPort = 5678;
        ...
        System.out.print("请输入本地Tcp代理端口(默认为5678):");
        String tcpProxyPortStr = scanner.nextLine();
        ...
        try (DatagramSocket udpSocket = new DatagramSocket(locolUdpSocketPort)) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        while ((dataLen = inputStream.read(byteBuf)) > 0) {
                            ...
                            //构造msg
                            ProxyMessage requestMsg = new ProxyMessage(ProxyMessage.P_TYPE_TRANSFER_TCP_OVER_UDP, dataLen, userId, realDataBytes);
                            byte[] dataBytes = UdpProxyMessageCodec.encode(requestMsg);
                            udpSocket.send(new DatagramPacket(dataBytes, dataBytes.length, clientAddress, port));
